...
- BaseButton, ViewportContainer's ``_unhandled_input()`` is now ``_unhandled_key_input()``.
- AnimationPlayer's ``add_animation()`` is now ``add_animation_library()``.
- BoxShape, RectangleShape's ``get_extents()`` is now ``get_size()``.
...
- BoxShape, RectangleShape's ``set_extents()`` is now ``set_size()``.
...
- PathFollow2D, PathFollow3D's ``offset()`` is now ``progress()``.
